...
-----QUERY 1 FOR DATA DICTIONARY
SELECT    c.TABLE_NAME,     c.COLUMN_NAME,     c.DATA_TYPE,     c.CHARACTER_MAXIMUM_LENGTH,     c.IS_NULLABLE,     c.COLUMN_DEFAULT,     ep.value AS COLUMN_DESCRIPTION
FROM    INFORMATION_SCHEMA.COLUMNS c    LEFT OUTER JOIN sys.extended_properties ep ON         ep.major_id = OBJECT_ID(c.TABLE_SCHEMA + '.' + c.TABLE_NAME) AND         ep.minor_id = c.ORDINAL_POSITION AND         ep.name = 'MS_Description'
ORDER BY    c.TABLE_NAME,     c.ORDINAL_POSITION


-----QUERY 2 FOR DATA DICTIONARY

-- IMPORTANT
USE IT_FIRM;  -- <<<<<<<<<<<< CHANGE THIS VALUE to Schema you want to dump
;
